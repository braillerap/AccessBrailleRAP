...
function wrap_unicode_back_translate_string (module)
    {
        return ((str, tableid) => {

            const widechar_size_bytes = 4;
            let len = str.length;
            let ptr = module._malloc ((len + 4) * widechar_size_bytes);
            
            // Convert js string to UTF32
            module.stringToUTF32(str, ptr, (len + 4) * widechar_size_bytes);
            
            // back translate string
            let result = module.ccall ("unicode_backtranslate_string", "number", ["number", "number", "number"], 
                                    [ptr, len+4, tableid]
                                    );
            console.log ("result:", result);
            module._free (ptr);                            
            
            // get back translation status
            let error = module.ccall ("unicode_translate_get_status", "number", [], 
                                    []
                                    );
            console.log ("error:", error);
            ...
            if (error === 1)
            {
              // translate back UTF32 to js string  
              rstr=module.UTF32ToString (result, (len + 4) * widechar_size_bytes * 2);
            }
            else
              rstr ="Error while translating " + error.toString()  
...
class libLouis 
{
    ...
    */
    ...
    init (module)
    {
        ...
        this.f_unicode_back_translate_string = wrap_unicode_back_translate_string(module);
    ...
    unicode_back_translate_string(str, tableid)
    {
        if (this.f_unicode_back_translate_string)
        {
            return this.f_unicode_back_translate_string (str, tableid);
        }
        return ""
    }
